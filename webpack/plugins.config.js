...
const htmlPlugins = pages.map(page => { // для многостраничного сайта. требуется: multipage.config.js
...
result.plugins = [
  ...
  //     favicon: './src/assets/favicon.svg', // Для браузеров которые поддерживают svg. В каталоге билда будет файл.svg. А для остальных браузеров используем плагин FaviconsWebpackPlugin.
  //     template: path.join(pathSrc, './pages/desktop-petstory.html'),
...
result.module = {
  rules: [
    { // CSS
      test: /\.(c|sa|sc)ss$/i, use: [
        devMode ? 'style-loader' : MiniCss.loader, // devMode
        'css-loader',
        // {
        //     loader: 'css-loader',
        //     options: {
...

        {
          ...
          options: {
            postcssOptions: {
              plugins: [require('postcss-preset-env')] // Подключение плагина, в котором включены авто префиксы. Опирается на 'browserslist'
            }
          }
        },
        'sass-loader', // в файл index.js: добавить: import './index.scss
        // {
    ...
    { // import 'file.html'
      test: /\.(html)$/i, // для работы и изображениями в html и pug, и обновления сервера отладки
      ...
      use: {
        ...
        options: {
          presets: [
            ...
            // '@babel/preset-react', // для работы в React и его файлами jsx
          ]
        }
      }
    },
